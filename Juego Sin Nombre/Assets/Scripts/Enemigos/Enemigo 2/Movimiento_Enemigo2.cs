...
public class Movimiento_Enemigo2 : MonoBehaviour
{
    ...
    private void Update() 
    {
        ...
        if(_iaPath.canSearch && _iaPath.canMove && _sprite.material.color != colorAtacando)
        {
            cambiarColor(colorAtacando);
        }
        else if(!_iaPath.canSearch && !_iaPath.canMove && _sprite.material.color != colorPrincipal)
        {
